...
interface WelcomeSectionProps {
  firstName: string;
  lastCourse: EnrolledCourse | null;
}

export function WelcomeSection({ firstName, lastCourse }: WelcomeSectionProps) {
  const resumeHref = lastCourse ? `/learn/${lastCourse.courseId}` : '/i-miei-corsi';
      ...
      <div className="relative z-10 flex flex-col sm:flex-row sm:items-end justify-between gap-4 px-6 py-6 lg:px-8 lg:py-7">
        <div>
          ...
          {lastCourse ? (
            <p className="text-text-secondary text-[0.88rem] mt-1">
              Stai studiando{' '}
              <span className="text-text-primary font-medium">{lastCourse.title}</span>
              {' '}— {lastCourse.progress}% completato.
            </p>
          ) : (
            <p className="text-text-secondary text-[0.88rem] mt-1">
              Esplora il catalogo e inizia il tuo primo corso.
            </p>
          )}
        </div>
        <div className="flex gap-2.5 shrink-0">
          ...
          >
            ...
            {lastCourse ? 'Riprendi Corso' : 'I miei corsi'}
          </Link>
          <Link
            href="/catalogo-corsi"
