import type { AdminCourseActivityItem, ActivityAccessType } from '@/lib/admin/queries';

interface CourseActivityFeedProps {
  items: AdminCourseActivityItem[];
}

const ACCESS_LABELS: Record<ActivityAccessType, { label: string; className: string }> = {
  single:           { label: 'Acquisto',      className: 'bg-accent-cyan/10 text-accent-cyan border-accent-cyan/20' },
  pro_subscription: { label: 'Abbonamento',   className: 'bg-accent-violet/10 text-accent-violet border-accent-violet/20' },
  free:             { label: 'Gratuito',       className: 'bg-accent-emerald/10 text-accent-emerald border-accent-emerald/20' },
  team:             { label: 'Team',           className: 'bg-accent-amber/10 text-accent-amber border-accent-amber/20' },
};
...
export function CourseActivityFeed({ items }: CourseActivityFeedProps) {
  if (items.length === 0) {
    return (
      <p className="text-[0.82rem] text-text-muted py-6 text-center">
        Nessuna attività registrata per questo corso.
      </p>
    );
  }
    ...
    <div className="space-y-0">
      ...
      <ol className="relative border-l border-border-subtle ml-3 space-y-0">
        {items.map((item, idx) => {
          ...
          const accessConf = item.accessType ? ACCESS_LABELS[item.accessType] : null;
              ...
              <div className="flex items-baseline gap-2 flex-wrap">
                {item.type === 'created' ? (
                  ...
                  <>
                    ...
                    {accessConf && (
                      <span className={`text-[0.65rem] font-semibold px-1.5 py-0.5 rounded border ${accessConf.className}`}>
                        {accessConf.label}
                      </span>
                    )}
                  </>
